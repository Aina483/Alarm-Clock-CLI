...
Design decision: a single JSON file, read fully and rewritten fully on each
...
class AlarmStore:
    ...
    def load(self) -> List[Alarm]:
        ...
        try:
            ...
            # A corrupted/partial file shouldn't crash the whole app; treat
            # it as empty and let the user re-add alarms. Loud enough to
            # notice, not so loud it blocks usage.
    ...
    def save(self, alarms: List[Alarm]) -> None:
        ...
        tmp_path.replace(self.path)  # atomic on POSIX, avoids a half-written file
